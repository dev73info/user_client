    ...
    <main class="page-shell custom-page-shell profile-page my-requirements-page">
        <section class="wallet-section my-requirements-summary">
            <div class="wallet-header">
                <div>
                    <h3>需求单概览</h3>
                    <small class="requirement-note">这里只展示你提交的需求单、支付交付进度和验收动作。</small>
                </div>
                <button class="ghost small" type="button" :disabled="requirementLoading || conversationLoading"
                    @click="refreshRequirementsPage">
                    {{ requirementLoading || conversationLoading ? '刷新中...' : '刷新需求单' }}
                </button>
            </div>
        ...
        <section class="wallet-section">
            <div class="wallet-header">
                ...
                <button class="ghost small" type="button"
                    @click="router.push({ name: 'requirement-hall' })">发布新需求</button>
            </div>
                ...
                <template v-for="item in myRequirements" :key="item.requirement_id">
                    <li class="requirement-row"
                        ...
                        <div class="requirement-main">
                            ...
                            <small v-if="item.status === 'rejected' && item.review_note"
                                class="requirement-note">驳回原因：{{
                        ...
                        <div class="requirement-actions">
                            ...
                            <button v-else-if="canComment(item.status) && !canComplete(item)" class="ghost small"
                                type="button" @click.stop="openCommentModal(item)">去评价</button>
                        </div>
                    </li>
                    ...
                    <li v-if="isVersionDeleteReviewExpanded(item)" class="requirement-review-card-row">
                        <section class="requirement-review-card" @click.stop>
                            <div class="requirement-review-card__header">
                                ...
                                <button class="ghost small" type="button"
                                    @click.stop="closeVersionDeleteReviewCard">收起</button>
                            </div>
                            <div class="requirement-review-card-list">
                                <article v-for="request in pendingResourceVersionDeleteRequests(item)"
                                    :key="request.version_id" class="requirement-review-item">
                                    <div class="requirement-review-item__main">
                                        ...
                                        <div class="requirement-review-item__actions">
                                            ...
                                            <button class="ghost small" type="button"
                                                :class="{ active: isRejectingVersion(request) }"
                                                :disabled="resourceVersionDeleteReviewLoadingId === item.requirement_id"
                                                @click.stop="openRejectVersion(request)">拒绝删除</button>
                                        </div>
                                    </div>
                                    <p class="requirement-review-card__hint danger">同意后该版本文件会被永久删除，且无法恢复。</p>
                                    <div v-if="isRejectingVersion(request)"
                                        class="requirement-review-item__reject-panel">
                                        <label
                                            class="requirement-review-card__field requirement-review-card__field--stacked">
                                            <span>拒绝原因</span>
                                            <textarea v-model="versionDeleteRejectNotes[request.version_id]"
                                                class="comment-input" rows="4" maxlength="500"
                                                placeholder="可选：说明拒绝删除该版本的原因"></textarea>
                                        ...
                                        <div class="requirement-review-card__actions">
                                            <button class="ghost small" type="button"
                                                @click.stop="cancelRejectVersion">取消</button>
                                            <button class="ghost small" type="button"
                                                :disabled="resourceVersionDeleteReviewLoadingId === item.requirement_id"
        ...
        <PublishModal :visible="editVisible" modalTitle="重新编辑需求" submitText="重新提交审核" loadingText="提交中..."
            v-model:publishTitle="editTitle" v-model:publishDescription="editDescription"
            v-model:publishBudget="editBudget" v-model:publishAcceptance="editAcceptance"
            v-model:publishPaymentMode="editPaymentMode" :publishLoading="editLoading" @close="closeEditModal"
            @submit="submitRequirementResubmit" />

        <RequirementConversationModal :visible="conversationVisible" :token="auth.token"
        ...
        <div v-if="completionConfirmVisible && completionConfirmTarget" class="modal-wrap"
            ...
            <section class="pay-modal" aria-label="需求完成确认弹窗">
                ...
                <div class="actions">
                    <button class="ghost" type="button" :disabled="completionLoading"
                        @click="closeCompletionConfirm">取消</button>
                    <button class="ghost" type="button" :disabled="completionLoading"
                        @click="submitRequirementCompletion">
